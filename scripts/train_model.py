# unbuffer python3 train_model.py  2>&1 | tee outfile1

# sudo docker attach --detach-keys="ctrl-x" claudio_train_gpu1
# sudo docker run -it --sig-proxy=false --detach-keys="ctrl-x" --name  claudio_train_gpu2 --gpus 1 -v /data2/claudios/projsplit:/datasets -v /data2/claudios/trace-python:/trace claudio-gpu bash

#  sudo docker run -it --sig-proxy=false --detach-keys="ctrl-x" --name  claudio_train_gpu1 --gpus 1 -v /data2/claudios/data:/datasets -v /data2/claudios/trace-python:/trace claudio-gpu bash
#  sudo docker run -it --sig-proxy=false --detach-keys="ctrl-x" --name  claudio_train_gpu1 --gpus 6 -v /data2/claudios/data:/datasets -v /data2/claudios/trace-python:/trace nvidia/cuda:11.4.2-cudnn8-runtime-ubuntu20.04 bash

# sudo docker build -t claudio/scipy-notebook:latest scipy-notebook/

#  sudo docker run --rm --gpus device=4 -v /data2/claudios/:/home/claudios/data2 -d -p 8888:8888 claudio/scipy-notebook:latest
# sudo docker run --rm -e GRANT_SUDO=yes -e JUPYTER_ENABLE_LAB=yes -e NB_UID="$(id -u)" -e NB_GID="$(id -g)" --user root --restart always -v /data2/claudios/:/home/claudios/data2 -d -p 8888:8888 claudio/scipy-notebook:latest
# sudo docker run \
#  --rm \
#  --user $(id -u) --group-add users \
#  -v /data2/claudios/:/home/jovyan/work
#  -e RESTARTABLE=yes
#  -p 8888:8888
#  claudio/scipy-notebook:latest
# -e JUPYTER_ENABLE_LAB=yes -e NB_UID="$(id -u)" -e NB_GID="$(id -g)" --user root --restart always -v /data2/claudios/:/home/claudios/data2 -d -p 8888:8888 claudio/scipy-notebook:latest
# sudo docker run --rm --sig-proxy=false --detach-keys="ctrl-x" --user $(id -u) --group-add users -v /data2/claudios/:/home/jovyan/work -e RESTARTABLE=yes -p 8888:8888 claudio/scipy-notebook:latest
# -*- coding: utf-8 -*-
"""Tokenizer-Quicktour
...
def preprocess_function(examples):
    return fast_tokenizer(
        examples["text"], max_length=128, truncation=True, padding=True
    )
...
from transformers import DataCollatorForWholeWordMask, TrainingArguments

data_collator = DataCollatorForWholeWordMask(
    tokenizer=fast_tokenizer, mlm=True, mlm_probability=0.15
)
training_args = TrainingArguments(
    "test_trainer_bert_pre",
    num_train_epochs=1,
...
trainer_eval = trainer.evaluate(
    encoded_dataset_test, ignore_keys=["text"], metric_key_prefix="no"
)
...
fill_mask = pipeline(
    "fill-mask", model="./test_trainer_bert_pre", tokenizer="./test_trainer_bert_pre"
)
result = fill_mask(
    "setSuccessorInMultimap [ENTRY] void com.google.common.collect.LinkedHashMultimap$ValueEntry [MASK] [EXIT]"
)
