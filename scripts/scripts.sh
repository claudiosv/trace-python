...
generate_project_report() {
    ...
    project_built=$(grep -c "BUILD SUCCESS" "$proj"/logs_X4PmlaxV/mvn_log_build_*.txt | wc -l)
    project_didnt_build=$(grep -c "BUILD FAILURE" "$proj"/logs_X4PmlaxV/mvn_log_build_*.txt | wc -l)
